...
class LinkColumn(ShowlinkMixin, tables.LinkColumn):
    def render(self, value, **kw):
        table = kw['table']
        show = not (self.show_link and not self.show_link(table.request))
        if show:
            return super(LinkColumn, self).render(value, kw['record'], kw['bound_column'])
        else:
            return value
    
class SelfLinkColumn(ShowlinkMixin, tables.Column):
    def render(self, value, **kw):
        table = kw['table']
        show = not (self.show_link and not self.show_link(table.request))
        if show:
            return self.render_link(value, **kw)
        else:
            return value
        
    def render_link(self, value, **kw):
        record = kw['record']
        table = kw['table']
